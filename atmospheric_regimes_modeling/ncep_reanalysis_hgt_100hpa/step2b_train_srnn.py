...
def make_ts_of_inputs_and_targets(pcs,kpcs):
    input_ts=torch.tensor(kpcs[...,:3],dtype=torch.get_default_dtype()) 
    target_ts=torch.tensor(np.concatenate([kpcs[...,:3]],axis=-1),dtype=torch.get_default_dtype())    
...
if __name__=='__main__':
    ...
    for n in range(100):
        for m in [7,8,9,10,11,12,13,14,15]:
