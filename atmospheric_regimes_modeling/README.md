...
- We use geopotential height (HGT) daily winter data:
    - generated by MPI-ESM-HR model in picontrol CMIP6 experiment
...
Here we assume that the following **ESM data** was downloaded in a dedicated folder (see the script contetns to modify the folder name):
...
Data preparation pipeline implemented (look the script for details):
 ...
 - Extracting the years 2198–2273, from the first April 1 to the last March 31 (75 years in total)
 ...
 - Evaluation data: repeat the above for the years 2274–2349
...
For the **NCEP reanalysis data**, the preprocessing is the same, except that the files to download are located at https://downloads.psl.noaa.gov//Datasets/ncep.reanalysis/Dailies/pressure, and the training time interval is 1950-2025 (no evaluation dataset, since we check the robustness to the interval change in the MPI-ESM-HR example).
...
Steps 3-5 are implemented as jupyter notebooks in each folder. See the comments there.
